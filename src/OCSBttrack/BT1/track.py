...
from OCSBttrack.BTcrypto import CRYPTO_OK
from T2T import T2TList
from OCSBttrack.subnetparse import IP_List, ipv6_to_ipv4, to_ipv4, is_valid_ip, is_ipv4
from OCSBttrack.iprangeparse import IP_List as IP_Range_List
from OCSBttrack.torrentlistparse import parsetorrentlist
from threading import Event, Thread
from OCSBttrack.bencode import bencode, bdecode, Bencached
from OCSBttrack.zurllib import urlopen, quote, unquote
...
from OCSBttrack.clock import clock
...
import signal
import re
import OCSBttrack.__init__
from OCSBttrack.__init__ import version, createPeerID
try:
    True
except:
    True = 1
...
defaults = [
    ('tracker_timeout', 0, "Time to stop tracker"),
    ('tracker_max_completed', 0, "Stop tracker via count of transfer completed"),
...
class Tracker:
    def __init__(self, config, rawserver):
        ...
        self.state = {}
        self.seedcount = {}
        self.tracker_inittime = time()
        self.tracker_timeout = config['tracker_timeout']
        self.tracker_max_completed = config['tracker_max_completed']
        ...
        rawserver.add_task(self.test_state_and_finish, 1)
        self.prevtime = clock()
    ...
    def get(self, connection, path, headers):
        ...
        try:
            ...
            if event == 'completed':
                self.completedno=self.completedno+1
                print "complete : %s" % self.completedno
    ...
    def test_state_and_finish(self):
        exit = 0
        elapsed = time() - self.tracker_inittime
        #print "config %s %s" % (self.tracker_max_completed, self.tracker_timeout)
        #print "show state"
        ...
        for k,v in self.state["completed"].items():
            ...
            if exit == 1:
                ...
                self.rawserver.set_finish()
        self.rawserver.add_task(self.test_state_and_finish, 5)
